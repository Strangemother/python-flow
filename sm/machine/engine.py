...
def run_all_flow(flow, *a, **kw):
    ...
    if run is False:
        print('Will not run_all_flow as flow.complete is True')
        return ()
    ...
    while run:
        ...
        if isinstance(result, Signal):
            print('  engine.run_all_flow received Signal return.')
...
def signal_check(flow, task, result):
    ...
    if is_stop_signal(flow, task, result):
        print(f'-    Stopping flow({flow}) to {result.state} because:', task.name, result.reason)
...
def run_flow_step(flow, *a, **kw):
    ...
    """
    if flow.complete:
        print('  Will not continue, flow complete')
        return

    print('  run_flow_step', flow.state)
    # At this point we can check the flow for a previous state.
    if flow.state == 'WAIT':
        print('  Flow state is WAIT, checking the existing.')
...
def step_check_continue_flow(flow, *a, **kw):
    ...
    print('  Assert a test of the current task.', TaskClass)
    ...
    if is_false:
        ...
        print('  Check returned bool "False", will not continue')
        return task, wait(reason=f'check assert ready: {not is_false}')

    if isinstance(res, Signal):
        print('  check continue received signal')
        stop = is_stop_signal(flow, task, res)
        if stop:
            print('  . Stop, storing result and not continuing index')
            set_flow_state(flow, res.state)
            store_task_result(flow, task, res)
            return task, res
    print('  check good - save flow step and continue', res)
...
def step_continue_flow(flow, *a, **kw):
    ...
    """
    print('  step_continue_flow')
    set_flow_running(flow)

    res, task = run_routine_index_task(flow.routine, flow.position)
...
def run_routine_index_task(routine, position=0):
